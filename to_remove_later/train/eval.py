...
class CustomEvaluation(VizualizationResults):
    ...
    def create_df_wind(self,
        ...
        labels = self.data.get_labels("speed", self.mode)
        inputs = self.data.get_inputs("speed", self.mode)
...
class Interpretability(VizualizationResults):
    ...
    def compute_feature_importance(self, mode, epsilon=0.01):
        ...
        inputs_speed = self.data.get_inputs("speed", mode)
        inputs_dir = self.data.get_inputs("dir", mode)

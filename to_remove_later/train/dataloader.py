...
from typing import Optional, Tuple, Union, Any, List, MutableSequence, Generator, Dict
...
class DataHolder:
    ...
    def __init__(self,
                 inputs: Union[pd.DataFrame, pd.Series, Union] = None,
                 length: Union[int, Union] = None,
                 labels: Union[pd.DataFrame, pd.Series, Union] = None,
                 names: Union[MutableSequence[str], None] = None,
                 mean: Union[np.array, None] = None,
                 std: Union[np.array, None] = None,
                 predicted: Union[pd.DataFrame, pd.Series, None] = None
                 ) -> None:
        self.inputs = inputs
        self.length = length
        self.labels = labels
...
class CategoryHolder:
    def __init__(self, dict_data_holder: Dict[str, DataHolder]) -> None:
        # Defined later
        self.train = None
        self.test = None
        self.val = None
        ...
        for key in dict_data_holder:
            setattr(self, key, dict_data_holder[key])
...
class CustomDataHandler:
    ...
    def __init__(self, config: dict) -> None:
        ...
        # self.speed hold data for speed network
        self.speed: Union[CategoryHolder, None] = None
        # self.speed hold data for direction network
        self.dir: Union[CategoryHolder, None] = None
    ...
    def _select_all_variables_needed(self,
        ...
        variables_needed = self.config["variables_needed"] if variables_needed is None else variables_needed
        if "alti-zs" in variables_needed:
            df["alti-zs"] = df["alti"] - df["ZS"]
            # todo do not remove "alti" and "zs" if  they are asked jointly with "alti-zs"
            variables_needed.remove("alti")
            variables_needed.remove("ZS")
    ...
    def unbalance_training_dataset(self) -> None:
        ...
        """
        raise NotImplementedError("This function needs to be refactored to take into "
                                  "account speed and dir specificities")
    ...
    def select_quick_test_stations(self,
                                   time_series: pd.DataFrame
                                   ) -> pd.DataFrame:
    ...
    def prepare_train_test_data(self,
        ...
        if self.config["quick_test"]:
            time_series = self.select_quick_test_stations(time_series)
        ...
        # Add month and hour
        if "month" in self.config["variables_needed"] or "hour" in self.config["variables_needed"]:
        ...
        if self.config["stations_val"]:
            types_of_data.append("val")
        if self.config.get("country_to_reject_during_training", False):
            types_of_data.append("other_countries")

        for category in ["speed", "dir"]:
            dict_data_holders = {}
            for type_of_data in types_of_data:

                inputs_data_holder = {
                    # e.g. time_series_train[self.config["input_variables"]]
                    "inputs": locals()[f"time_series_{type_of_data}"][self.config[f"input_variables_{category}"]],
                    # e.g. len(self.inputs_train)
                    "length": len(getattr(self, f"inputs_{type_of_data}")),
                    # e.g. time_series_train[self.config["labels"]]
                    "labels": locals()[f"time_series_{type_of_data}"][self.config[f"label_{category}"]],
                    # e.g. self.names_train = time_series_train["name"]
                    "names": locals()["name"]
                }

                if (self.config.get("standardize", True)) and (type_of_data == "train"):
                    inputs_data_holder["mean_standardize"] = inputs_data_holder["inputs"].mean()
                    inputs_data_holder["std_standardize"] = inputs_data_holder["inputs"].std()
                    # todo refactor self.mean_standardize and self.std_standardize

                dict_data_holders[type_of_data] = DataHolder(**inputs_data_holder)

            # e.g. self.speed = CategoryHolder({"train": {"inputs": time_series_train[config["input_variables"]]})
            setattr(self, category, CategoryHolder(dict_data_holders))

        assert self.speed.train.length == self.dir.train.length
        assert self.speed.train.length == len(self.speed.train.names)
        assert self.dir.train.length == len(self.dir.train.names)
        assert len(self.dir.train.names) == len(self.speed.train.names)
        assert self.dir.train.names[0] == self.speed.train.names[0]
        assert self.dir.train.names[1] == self.speed.train.names[1]
        assert self.dir.train.names[2] == self.speed.train.names[2]
        assert self.dir.train.names[-1] == self.speed.train.names[-2]
        assert self.dir.train.names[-2] == self.speed.train.names[-1]
        assert self.speed.test.length == self.dir.test.length
        assert self.dir.test.length == len(self.dir.test.names)
        assert self.speed.test.length == len(self.speed.test.names)
        assert len(self.dir.test.names) == len(self.speed.test.names)
        assert self.dir.test.names[0] == self.speed.test.names[0]
        assert self.dir.test.names[1] == self.speed.test.names[1]
        assert self.dir.test.names[2] == self.speed.test.names[2]
        assert self.dir.test.names[-1] == self.speed.test.names[-2]
        assert self.dir.test.names[-2] == self.speed.test.names[-1]
    ...
    # todo refactor use of get_inputs
    # here I worked
    def get_inputs(self,
                   category: str,
                   mode: str
                   ) -> Union[pd.Series, pd.DataFrame]:
        return getattr(getattr(self, category), mode).inputs

    # todo refactor use of get_length
    def get_length(self,
                   mode: str
                   ) -> float:
        return getattr(getattr(self, "speed"), mode).length

    # todo refactor use of get_labels
    def get_labels(self,
                   category: str,
                   mode: str
                   ) -> pd.DataFrame:
        return getattr(getattr(self, category), mode).labels

    # todo refactor use of get_names
    def get_names(self,
                  mode: str,
                  unique: bool = False
                  ) -> MutableSequence:
        names = getattr(getattr(self, "speed"), mode).names
    ...
    # todo refactor use of get_mean
    def get_mean(self, category: str) -> MutableSequence[float]:
        # e.g. self.speed.mean_standardize
        return getattr(self, category).mean_standardize

    # todo refactor use of get_std
    def get_std(self, category: str) -> MutableSequence[float]:
        # e.g. self.dir.std_standardize
        return getattr(self, category).std_standardize

    # todo refactor use of get_tf_map
    def get_tf_map(self,
                   names_map: MutableSequence,  # e.g. ["topos", "aspect", "tan_slope", ...]
                   mode: str,
                   names: Union[MutableSequence[str], None] = None,  # names of the stations
        ...
        if names is None:
            names = self.get_names(mode, unique=False)  # "speed" or "dir is the same here
    ...
    # todo refactor use of get_tf_mean_std
    def get_tf_mean_std(self,
        ...
        mean = self.get_mean(category)
        std = self.get_std(category)

        mean = tf.data.Dataset.from_generator(MeanGenerator(mean,
                                                            length),
                                              output_types=tf.float32,
                                              output_shapes=(self.config[f"nb_input_variables_{category}"],))
    ...
    # todo refactor use of get_tf_zipped_inputs
    def get_tf_zipped_inputs(self,
                             mode: str,
                             inputs_speed: Union[pd.Series, pd.DataFrame] = None,  # optional
                             inputs_dir: Union[pd.Series, pd.DataFrame] = None,  # optional
                             names: MutableSequence[str] = None,  # optional
                             output_shapes: MutableSequence = [140, 140, 1]
                             ) -> tf.data.Dataset:

        output_shapes[2] = len(self.config["map_variables"])

        if inputs_speed is None:
            inputs_speed = self.get_inputs("speed", mode)
        if inputs_dir is None:
            inputs_dir = self.get_inputs("dir", mode)

        if hasattr(inputs_speed, "values"):
            inputs_speed = inputs_speed.values
        if hasattr(inputs_dir, "values"):
            inputs_dir = inputs_dir.values

        inputs_speed = tf.data.Dataset.from_tensor_slices(inputs_speed)
        inputs_dir = tf.data.Dataset.from_tensor_slices(inputs_dir)

        if self.config["standardize"]:
            mean_speed, std_speed = self.get_tf_mean_std("speed", mode)
            mean_dir, std_dir = self.get_tf_mean_std("dir", mode)
            return tf.data.Dataset.zip((self.get_tf_map(names_map=self.config["map_variables"],
                                                        mode=mode,
                                                        names=names,
    ...
    # todo refactor use of get_tf_zipped_inputs_labels
    def get_tf_zipped_inputs_labels(self,
                                    ...
                                    inputs_dir: Union[pd.Series, pd.DataFrame] = None,  # optional
                                    names: MutableSequence[str] = None,  # optional
                                    labels: Union[pd.Series, pd.DataFrame] = None,  # optional
                                    output_shapes: MutableSequence = [140, 140, 1],
                                    ) -> tf.data.Dataset:
        if labels is None:
            labels = self.get_labels(category, mode)
    ...
    def get_time_series(self,
        ...
        # todo refactor this function for custom devine
        """time_series = self.loader.load_time_series_pkl()
        if prepared:
            ...
            return time_series
            """
        raise NotImplementedError("Need to refacor this function for speed and dir")
    ...
    def get_predictions(self,
                        ...
                        mode: str):
        try:
            return getattr(getattr(self, category), f"_{mode}").predicted
        except AttributeError:
            try:
                return getattr(getattr(self, category), f"{mode}").predicted
            except AttributeError:
    ...
    # todo refactor use of get_batched_inputs_labels
    def get_batched_inputs_labels(self,
                                  category: str,
                                  mode: str,
                                  inputs_speed: Union[pd.Series, pd.DataFrame] = None,
                                  inputs_dir: Union[pd.Series, pd.DataFrame] = None,
        ...
        output_shapes[2] = len(self.config["map_variables"])
        dataset = self.get_tf_zipped_inputs_labels(category,
                                                   mode,
                                                   inputs_speed=inputs_speed,
                                                   inputs_dir=inputs_dir,
                                                   names=names,
    ...
    # todo refactor use of set_predictions
    def set_predictions(self,
        ...
        names = self.get_names(mode, unique=False)
        df, mode_str = self.results_setter.prepare_df_results(results, names, mode=mode, str_model=str_model)

        self.speed.test.predicted = 0

        setattr(getattr(getattr(self, category), mode), "predicted", df)
        ...
        if self.config.get("get_intermediate_output", False):
            ...
            setattr(getattr(getattr(self, category), mode), "predicted_int", df)

    def _set_is_prepared(self) -> None:
        self.is_prepared = True

    # todo refactor use of add_model
    def add_model(self,
                  model: str,
                  category: str,
                  mode: str = "test"
        ...
        setattr(getattr(getattr(self, category), mode), f"predicted{model}", predictions)

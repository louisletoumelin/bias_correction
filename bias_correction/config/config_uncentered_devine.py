...
# Architecture
config["details"] = "test_uncentered"  # Str. Some details about the experiment
config["global_architecture"] = "uncentered_devine"  # Str. Default="ann_v0", "dense_only", "dense_temperature", "devine_only", "double_ann"
config["restore_experience"] = False
config["random_idx"] = True

# ann_v0
config["disable_training_cnn"] = False
config["type_of_output"] = "uncentered_u_v"  # Str
...
config["kwargs_optimizer"] = {"clipvalue": 0.5}  # Dict.
config["min_idx_uncentered"] = -20
config["max_idx_uncentered"] = 20
...
# Regularization
config["add_regularization_devine"] = True
config["rate_devine_regularization"] = 1
...
config["quick_test_stations"] = ["Col du Lac Blanc"]
# config["quick_test_stations"] = ["ALPE-D'HUEZ", 'Col du Lac Blanc', 'SOUM COUY-NIVOSE', 'SPONDE-NIVOSE']

# Input variables
config["input_variables"] = ["idx_x", "idx_y", "Wind","Wind_DIR"]
#config["input_speed"] = ["idx_x", "idx_y", "Wind","Wind_DIR"]
#config["input_dir"] = ['Wind', 'Wind_DIR']
# todo write a test that checks that topos, aspect and tan_slope are in the correct order
config["map_variables"] = ["topos"]
config["compute_product_with_wind_direction"] = False
...
# Labels
config["labels"] = ["U_obs", "V_obs"]  # ["vw10m(m/s)"] or ["U_obs", "V_obs"] or ['T2m(degC)'] or ['winddir(deg)']
...
# Callbacks
config["callbacks"] = ["ModelCheckpoint", "CSVLogger", "TensorBoard"]  # "FeatureImportanceCallback", "EarlyStopping",
...
config["kwargs_callbacks"] = {"ReduceLROnPlateau": {"monitor": "val_loss",
                              ...
                              "TensorBoard_v1": {"profile_batch": '20, 50',
                                                 "histogram_freq": 1,
                                                 "write_grads": True},
...
# Custom loss
config["loss"] = "mse"  # Str. Default=mse. Used for gradient descent

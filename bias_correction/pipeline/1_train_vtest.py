...
from bias_correction.config.config_train_vtest import config
...
if not config["restore_experience"]:
    ...
    print(cm.model_is_built)
    print(cm.model_is_compiled)

    with tf.device('/GPU:0'), timer_context("fit"):
        # todo WARNING change test in train
        _ = cm.fit_with_strategy(data_loader.get_batched_inputs_labels(mode="test"),

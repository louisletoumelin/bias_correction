...
inputs = data_loader.get_time_series(prepared=False)
"""
with tf.device('/GPU:0'):

    # Predict
    with timer_context("Predict test set"):
        inputs = data_loader.get_inputs("test")
        inputs = inputs.iloc[0]
        inputs_test = data_loader.get_tf_zipped_inputs(inputs=inputs).batch(len(inputs))
